...
# ============================
# 檢測參數
CONFIDENCE_THRESHOLD = 0.8    # 匹配信心度閾值 (0.0-1.0)
MIN_DETECTION_SIZE = 20       # 最小檢測尺寸（像素）

# 多尺度匹配參數  
SCALE_MIN = 0.3              # 最小縮放比例
SCALE_MAX = 2.0              # 最大縮放比例
SCALE_STEP = 0.02            # 縮放步進

# NMS參數
NMS_THRESHOLD = 0.5          # 重疊閾值

# 視覺化參數
BBOX_COLOR = (255, 0, 0)     # 邊界框顏色 (BGR)
BBOX_THICKNESS = 1           # 邊界框粗細
...
def calculate_iou(box1, box2):
    """
    計算兩個边界框的IoU（Intersection over Union）
    """
...
def non_max_suppression(detections, threshold=NMS_THRESHOLD):
    ...
    if len(detections) <= 1:
        for detection in detections:
            detection['max_overlap'] = 0.0
        print(f"結果: 保留全部 {len(detections)} 個檢測")
        return detections
    ...
    for i, detection in enumerate(sorted_detections):
        keep = True
        current_box = [detection['location'][0], detection['location'][1], 
                      detection['location'][0] + detection['size'][0], 
                      detection['location'][1] + detection['size'][1]]
        
        # 檢查與已保留檢測的重疊
        for kept_detection in filtered_detections:
            kept_box = [kept_detection['location'][0], kept_detection['location'][1],
                       kept_detection['location'][0] + kept_detection['size'][0],
                       kept_detection['location'][1] + kept_detection['size'][1]]
            
            overlap = calculate_iou(current_box, kept_box)
            if overlap > threshold:
                print(f"移除 {detection['template_name']} (信心度: {detection['confidence']:.3f})")
                print(f"  原因: 與 {kept_detection['template_name']} 重疊 {overlap:.3f} > {threshold}")
                removed_count += 1
                keep = False
                break
        ...
        if keep:
            ...
            for other_detection in detections:
                if detection != other_detection:
                    other_box = [other_detection['location'][0], other_detection['location'][1],
                               other_detection['location'][0] + other_detection['size'][0],
                               other_detection['location'][1] + other_detection['size'][1]]
                    overlap = calculate_iou(current_box, other_box)
                    max_overlap = max(max_overlap, overlap)
            
            detection['max_overlap'] = max_overlap
            filtered_detections.append(detection)
    
    print(f"結果: 移除 {removed_count} 個重疊檢測，保留 {len(filtered_detections)} 個")
    ...
    for template_name, template in templates.items():
        ...
        if match_result and match_result['confidence'] >= CONFIDENCE_THRESHOLD:
            ...
            if w >= MIN_DETECTION_SIZE and h >= MIN_DETECTION_SIZE:
                ...
                print(f"  ✓ {template_name} (信心度: {match_result['confidence']:.3f}, 尺寸: {w}×{h})")
            else:
                print(f"  ✗ {template_name} (尺寸過小: {w}×{h})")
        elif match_result:
            # 有檢測結果但未達信心度閾值，顯示最大信心度
            print(f"  ✗ {template_name} (信心度: {match_result['confidence']:.3f}，未達信心度閾值)")
        else:
            print(f"  ✗ {template_name} (無檢測結果)")
        
    
    if all_detections:
        print(f"\n檢測完成，總計 {len(all_detections)} 個檢測")
    
    # 執行非極大值抑制
    if all_detections:
        print(f"NMS處理: {len(all_detections)} 個檢測")
        ...
        for template_name in template_names:
            ...
            if found_detections:
                ...
                report_data.append({
                    ...
                    'IoU': f"{best_detection.get('max_overlap', 0.0):.3f}"
                })
            else:
                # 沒有檢測到
                report_data.append({
...
def process_all_targets():
    ...
    for target_file in target_files:
        ...
        result = save_results(result_image, detections, target_filename)
    ...
    print(f"檢測報告已保存: {os.path.join(RESULT_PATH, 'detection_report.csv')}")
